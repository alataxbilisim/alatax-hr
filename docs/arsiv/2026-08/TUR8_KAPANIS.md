...
| `employees.position` nasıl saklanıyor? | Tur8 anında string + code dual-write; **sonrasında** `position_id` FK eklendi (`2026_08_05_220000`) ve string kolon drop edildi (`2026_08_06_010000`) — SSOT = `position_id` |
| `EmployeeFormEnginePage` aynı desen mi? | Evet: Select `value = code` (zaten) |

### Yapılan (Tur8 — FE/Resource; FK sonraki turda)
...
**Sonraki tur (Faz 6 öncesi kapatma — güncel durum):** `employees.position_id` FK → `positions.id` migration + backfill; dual-write kaldırıldı; string kolon drop (`2026_08_06_010000`). SSOT = `position_id`. Detay: `docs/FAZ6_ONCESI_KAPATMA.md` §3, `docs/FAZ_G_KAPANIS_OZET.md`.
...
**Doğru davranış** (`docs/SISTEM_ISLEYIS.md` § Sistem panosu): sistem panosu salt okunur; **Kopyala** ile firma panosu oluşur; layout orada saklanır.
