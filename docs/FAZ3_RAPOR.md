...
## Son dalga — Sidebar daraltma + Portal ölçek (12 Temmuz 2026)

**Şablon:** TASARIM_REHBERI Bölüm 5 (layout) + Bölüm 8 (Portal ölçek).

### Sidebar (Company)
| Özellik | Durum |
|---------|--------|
| ContextSidebar geniş | **216px** (`--context-sidebar-width`) |
| Daraltılmış | **48px** — harf ikon + `title` tooltip; gezinme çalışır |
| ModuleRail | **56px** sabit |
| Tercih | `themeSlice.sidebarOpen` → `localStorage.contextSidebarExpanded` + `user.preferences.contextSidebarExpanded` |
| Toggle | header hamburger + sidebar chevron + aynı modüle tekrar tık |

1366 hedefi: rail 56 + collapsed 48 → içerik ~1262px (≥1240).
...
| Sidebar | 260→**220px** |
| Header | 64→**52px** |
| Body/btn/input | ~13px / min-height **34px** |
| Kart/nav padding | kompakt |
| Bootstrap | `--bs-body-font-size` + btn/input değişkenleri; **tam shared DS geçişi yok** (Faz 8) |

### Doğrulama
- 3 SPA lint + build: **0**
- Faz 2 portal.access / self-scope: dokunulmadı

**DUR:** Kullanıcı görsel kontrol — **sidebar toggle** (daralt/genişlet + tercih kalıcılığı) + **Portal login** gezinti. Commit yok.

Sonraki: uçtan uca test → cilalama → Faz 3 kapanış + main merge.
